...
/**
 * @desc    Get all public/active social links
...
export const getPublicSocialLinks = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        const socialLinks = await SocialLink.find({ status: 'active' }).sort({ createdAt: 'asc' });

        res.status(200).json({ socialLinks });
    } catch (error: any) {
        console.error('[Public SocialLink Controller] Fetch Error:', error.message);
        res.status(500).json({ message: 'Failed to fetch public social links' });
    }
};
